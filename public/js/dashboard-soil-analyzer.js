...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initCharts() {
    ...
    if (document.getElementById('phTrendChart')) {
      const phContainer = document.getElementById('phTrendChart').parentElement;
      
      // Set fixed height to the container
      phContainer.style.height = '300px';
    ...
    if (document.getElementById('nutrientBarChart')) {
      const nutrientContainer = document.getElementById('nutrientBarChart').parentElement;
      
      // Set fixed height to the container
      nutrientContainer.style.height = '300px';
      ...
      nutrientChart = new Chart(nutrientCtx, {
        ...
        options: {
          responsive: true,
          maintainAspectRatio: true, // Important to keep aspect ratio
          scales: {
            y: {
              beginAtZero: true,
              max: 100,
  ...
  const updateDashboardStats = (newSoil) => {
    ...
    // For immediate visual feedback before reload
    if (totalSamplesStat) {
    ...
    // Show temporary data update - will be replaced by server data after reload
    if (waterLevelStat) waterLevelStat.textContent = `${newSoil.waterLevel.toFixed(1)}%`;
    if (phLevelStat) phLevelStat.textContent = newSoil.phLevel.toFixed(1);
    if (soilHealthStat) soilHealthStat.textContent = `${newSoil.soilHealth.toFixed(1)}%`;
    
    // Force browser to completely reload from server, ignoring cache
    setTimeout(() => {
      // Add cache-busting parameter to prevent any caching
      const currentUrl = new URL(window.location.href);
      currentUrl.searchParams.set('_', timestamp);
      
      // Use location.replace to prevent history entry
      window.location.replace(currentUrl.toString());
    }, 1500);
  };
